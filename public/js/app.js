...
async function init() {
  ...
  renderQuestion();
  updateStats();
  if (window.Battle) window.Battle.init(App);
}
...
function bindUi() {
  ...
  $("undo-btn").onclick = () => App.drawingTools.undo();
  $("redo-btn").onclick = () => App.drawingTools.redo();
  $("clear-draw-btn").onclick = () => App.drawingTools.clear();
...
function createChart() {
  ...
  App.chart = LightweightCharts.createChart(chartEl, {
    ...
    timeScale: { borderColor: "rgba(148,163,184,.25)", timeVisible: false },
  ...
  App.chart.priceScale("volume").applyOptions({ scaleMargins: { top: .82, bottom: 0 } });

  const canvas = $("draw-canvas");
  const wrap = $("chart-wrap");
  App.drawingTools = new DrawingTools({ chart: App.chart, series: App.candleSeries, canvas, container: wrap });
  App.drawingTools.enable(false);

  new ResizeObserver(() => {
    const rect = chartEl.getBoundingClientRect();
    App.chart.applyOptions({ width: Math.floor(rect.width), height: Math.floor(rect.height) });
...
async function loadChallenge() {
  ...
  if (!res.ok) throw new Error("daily_challenge.json 載入失敗");
...
async function loadKlines(path) {
  const res = await fetch(`${path}?t=${Date.now()}`);
  if (!res.ok) throw new Error(`K 線資料載入失敗：${path}`);
...
async function renderQuestion() {
  ...
  const visibleEnd = Math.min(start + visibleBars, App.currentData.length - revealBars - 1);
  const revealEnd = Math.min(visibleEnd + revealBars, App.currentData.length - 1);
  ...
  $("question-title").innerText = `第 ${App.questionIndex + 1} / ${App.challenge.questions.length} 題：判斷後 ${revealBars} 根 K`;
  ...
  if (App.mode === "backtest") setupBacktest(false);
  updateStats();
}
...
function setChartData(candles) {
  ...
  App.volumeSeries.setData(candles.map(x => ({
    time: x.time,
    value: x.volume || 0,
    color: x.close >= x.open ? "rgba(34,197,94,.32)" : "rgba(239,68,68,.32)"
  })));
  requestAnimationFrame(() => App.drawingTools.resize());
}
...
function computeOutcome() {
  ...
  else if (ret < -threshold) direction = "down";
  return { direction, ret, maxUp, maxDown, baseClose: base.close, lastClose: last.close };
}

function labelDirection(d) {
  return d === "up" ? "看漲" : d === "down" ? "看跌" : "盤整";
}
...
function submitAnswer(answer) {
  ...
  App.answers.push({ question: App.questionIndex + 1, answer, correct: out.direction, gained, ticker: q.ticker, ret: out.ret });
  ...
  if (App.mode === "battle" && window.Battle) window.Battle.submitCurrentScore(App.score, App.hits, App.answers);
}
...
function showResult(correct, answer, out, gained) {
  ...
  box.innerHTML = `
    <div class="flex items-center justify-between gap-2 mb-2">
      <h3 class="font-black text-lg ${correct ? "text-emerald-300" : "text-rose-300"}">${correct ? "判斷正確" : "判斷失誤"}</h3>
      <span class="text-xs px-3 py-1 rounded-full bg-slate-800 border border-slate-700">+${gained} 分</span>
    </div>
    <div class="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs font-bold">
      <div class="bg-slate-900 border border-slate-700 rounded-xl p-3"><div class="text-slate-500">你的答案</div><div class="mt-1">${labelDirection(answer)}</div></div>
      <div class="bg-slate-900 border border-slate-700 rounded-xl p-3"><div class="text-slate-500">實際方向</div><div class="mt-1">${labelDirection(out.direction)}</div></div>
      <div class="bg-slate-900 border border-slate-700 rounded-xl p-3"><div class="text-slate-500">收盤變化</div><div class="mt-1 ${out.ret >= 0 ? "text-emerald-300" : "text-rose-300"}">${pct(out.ret)}</div></div>
      <div class="bg-slate-900 border border-slate-700 rounded-xl p-3"><div class="text-slate-500">區間振幅</div><div class="mt-1">上 ${pct(out.maxUp)} / 下 ${pct(out.maxDown)}</div></div>
    </div>
    <p class="text-sm text-slate-400 mt-3">${feedbackText(correct, answer, out)}</p>
  `;
...
function feedbackText(correct, answer, out) {
  if (correct) return "這題你抓到主要方向。重點不是猜對一次，而是累積大量樣本後檢查自己的誤判類型。";
  if (answer === "up" && out.direction === "down") return "你偏向追強，但後段收盤轉弱。下次注意突破後是否能守住關鍵低點。";
  if (answer === "down" && out.direction === "up") return "你被前面的壓力感影響，但後段買盤延續。下次注意低點是否沒有再破。";
  if (out.direction === "range") return "這段主要是盤整，方向感不強。裸 K 練習也要學會承認沒有明確方向。";
...
function finishChallenge() {
  const title = App.score >= 90 ? "裸 K 老狐狸" : App.score >= 70 ? "不追高戰士" : App.score >= 45 ? "盤感修煉中" : "被洗出去的人";
  const box = $("result-box");
  box.innerHTML += `
    <div class="mt-4 rounded-2xl border border-blue-500/30 bg-blue-500/10 p-4">
      ...
      <p class="text-sm text-slate-300 mt-1">命中 ${App.hits}/${App.challenge.questions.length}，總分 ${App.score}。</p>
    </div>
  `;
  saveDailyResult();
}
...
function saveDailyResult() {
  ...
  localStorage.setItem(key, JSON.stringify({ score: App.score, hits: App.hits, answers: App.answers, saved_at: new Date().toISOString() }));
}
...
function copyShareText() {
  const total = App.challenge?.questions?.length || 5;
  const text = `我剛完成今日裸 K 五連戰\n命中：${App.hits}/${total}\n分數：${App.score}\n\n不看消息、不看指標，只看 K 線。\n${cfg.siteUrl || "https://daydayquiz.com"}`;
  navigator.clipboard.writeText(text).then(() => alert("戰績文已複製"));
}
...
function backtestBuy() {
  ...
  bt.cash -= qty * price;
  bt.avgCost = bt.shares > 0 ? ((bt.avgCost * bt.shares) + (price * qty)) / (bt.shares + qty) : price;
  bt.shares += qty;
  bt.trades.push({ type: "buy", idx: bt.idx, price, qty });
...
function backtestSell() {
  ...
  bt.trades.push({ type: "sell", idx: bt.idx, price, qty });
  bt.shares = 0;
  bt.avgCost = 0;
...
function backtestNext() {
  ...
  else showBacktestStatus("已前進一根 K。", false);
}
...
function showBacktestStatus(message, force=true) {
  const bt = App.backtest;
  if (!bt) return;
  const price = currentBacktestPrice();
  ...
  box.innerHTML = `
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs font-bold">
      <div class="bg-slate-900 border border-slate-700 rounded-xl p-3"><div class="text-slate-500">目前價格</div><div>${price.toFixed(2)}</div></div>
      <div class="bg-slate-900 border border-slate-700 rounded-xl p-3"><div class="text-slate-500">現金</div><div>${money(bt.cash)}</div></div>
      <div class="bg-slate-900 border border-slate-700 rounded-xl p-3"><div class="text-slate-500">持股</div><div>${money(bt.shares)}</div></div>
      <div class="bg-slate-900 border border-slate-700 rounded-xl p-3"><div class="text-slate-500">權益變化</div><div class="${ret >= 0 ? "text-emerald-300" : "text-rose-300"}">${pct(ret)}</div></div>
    </div>
  `;
}
